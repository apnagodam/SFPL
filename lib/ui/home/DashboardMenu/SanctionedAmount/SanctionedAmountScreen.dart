...
class _SanctionedamountscreenState
  ...
  var schemeProvider = StateProvider<SchemeType?>((ref) => null);
                  ...
                  child: ref.watch(bankListProvider).when(
                      data: (data) => Column(
                            children: [
                              ...
                              if (appliedData.schemeType.toString() == '2')
                                DropdownSearch<SchemeType?>(
                                  validator: (value) {
                                    if (value == null || value.label.isEmpty) {
                                      return 'Please select Sanction type';
                                    }
                                    return null;
                                  },
                                  popupProps: PopupProps.menu(
                                  ...
                                  itemAsString: (SchemeType? u) =>
                                      "${u?.label}",
                                  onChanged: (SchemeType? data) => ref
                                      .watch(schemeProvider.notifier)
                                      .state = data,
                                  dropdownDecoratorProps:
                                      const DropDownDecoratorProps(
                                    dropdownSearchDecoration: InputDecoration(
                              ...
                              if (ref.watch(schemeProvider)?.type == 2)
                                ElevarmTextInputField(
                              ...
                              ElevarmPrimaryButton.text(
                                text: "Submit",
                                onPressed: () {
                                  if (ref.watch(schemeProvider) == null) {
                                    showErrorDialog(context,
                                        titleText: 'Sanction Type',
                                        messageText:
                                            'Please Select Sanction Type');
                                  } else {
                                    if (ref.watch(schemeProvider)?.type == 2) {
